...
export class PlaybackEngine {
  constructor({ audio, video, onChange = () => {}, onError = () => {}, contextFactory, wait }) {
    ...
    for (const [kind, element] of Object.entries(this.media)) {
      element.addEventListener('play', () => {
        ...
        for (const [otherKind, other] of Object.entries(this.media)) {
          if (otherKind !== kind && !other.paused) { this.ramp(otherKind, 0); other.pause(); }
        }
        this.active = kind;
        this.onChange();
      });
      ...
      element.addEventListener('error', () => {
        if (element.error) this.onError('媒体加载失败，请检查网络后重新点击播放。');
      });
  ...
  unlock() {
    ...
    return this.context.state !== 'running' ? this.context.resume() : Promise.resolve();
  }
  ...
  play(kind, source, selected) {
    return this.command(async current => {
      ...
      this.onChange();
      await element.play();
      if (!current()) { this.ramp(kind, 0); element.pause(); return; }
      this.ramp(kind, 1);
    });
  }
  pause(kind) {
  ...
  seek(kind, seconds, resumeOverride) {
    return this.command(async current => {
      ...
      const target = clamp(seconds, 0, Math.max(0, element.duration - .015));
      if (Math.abs(element.currentTime - target) > .001) {
        await new Promise(resolve => {
          const finish = () => { clearTimeout(timeout); element.removeEventListener('seeked', finish); resolve(); };
  ...
  setVolume(value) {
    ...
    if (this.master) this.master.gain.setTargetAtTime(this.muted ? 0 : this.volume, this.context.currentTime, .018);
    this.onChange();
  }
  ...
  async preloadSfx(paths) {
    ...
    this.rawSfx = Object.fromEntries(await Promise.all(Object.entries(paths).map(async ([key, path]) => {
      try { const response = await fetch(path); if (!response.ok) throw new Error('SFX unavailable'); return [key, await response.arrayBuffer()]; }
  ...
  async sfx(name) {
    ...
    try {
      if (!this.buffers[name] && this.rawSfx?.[name]) this.buffers[name] = await this.context.decodeAudioData(this.rawSfx[name].slice(0));
      if (!this.buffers[name] || serial !== this.sfxSerial || !this.sfxEnabled) return;
      this.stopSfx();
      ...
      gain.gain.linearRampToValueAtTime(name === 'notification' ? .28 : .19, now + .005);
      gain.gain.setValueAtTime(name === 'notification' ? .28 : .19, now + Math.max(.006, duration - .015));
      gain.gain.linearRampToValueAtTime(0, now + duration);
      source.connect(gain); gain.connect(this.master); source.start();
      source.onended = () => { source.disconnect(); gain.disconnect(); if (this.sfxVoice?.source === source) this.sfxVoice = null; };
      this.sfxVoice = { source, gain };
    } catch { /* A missing interface effect must never block media playback. */ }
  }
  stopSfx() {
    ...
    gain.gain.linearRampToValueAtTime(0, now + .005);
    try { source.stop(now + .006); } catch { /* Already ended. */ }
    this.sfxVoice = null;
  }
}
